...
export default function BiayaLainnya() {
  ...
  async function fetchData() {
    setLoading(true);
    const { data: biayaData, error } = await supabase
      .from("biaya_lainnya")
      .select("*")
  ...
  function validateForm() {
    if (!form.nominal || isNaN(form.nominal) || Number(form.nominal) <= 0) {
          ...
          <table className="w-full border-collapse table-auto text-left">
            <thead className="bg-indigo-200 dark:bg-indigo-700">
              <tr  className="bg-indigo-600 text-white">
                <th className="border border-gray-300 dark:border-gray-600 px-4 py-2">Nominal (Rp)</th>
            ...
            <tbody>
              {data.map(({ id, nominal, tanggal, catatan }) => (
                <tr
                  key={id}
                ...
                >
                  ...
                  <td className="px-4 py-2 text-center space-x-2">
                    <button
                      ...
                      aria-label={`Edit data nominal Rp ${nominal}`}
                    >
                      Edit
                    </button>
                    <button
                      ...
                      aria-label={`Hapus data nominal Rp ${nominal}`}
                    >
                      Hapus
                    </button>
                  </td>
